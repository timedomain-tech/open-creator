...
- [x] **コミュニティハブ**：より広いコミュニティからのスキルを共有し、利用します。 [huggingface_hub](docs/skill-library-hub.md) をサポート。 `langchain_hub` はまだです。
...
```
注意：Huggingfaceに保存する前に、Huggingfaceの認証方法を設定する必要があります。
ユーザーアクセストークンによる認証を推奨します。以下の手順に従ってください：

1. あなたのHuggingfaceアカウントにログインします。
2. 右上角のユーザーメニューから「Settings」を選択します。
3. 左側のナビゲーションバーで「API Tokens」を選択します。
4. 「New Token」ボタンをクリックして新しいアクセストークンを作成します。
5. トークンの目的を識別するための記述的な名前を入力します。
6. 適切なアクセス権限を選択し、スキルのアップロードと管理のための十分な権限を持っていることを確認します。
7. 「Create」ボタンをクリックしてトークンを生成します。
8. 生成されたアクセストークンをコピーし、安全に保存します。
9. ターミナルで「huggingface-cli login」と入力します。
10. 以前にコピーしたアクセストークンを入力し、エンターキーを押します。
このアクセストークンを使用して認証を行い、スキルの保存とアップロードに使用できるようになりました。save関数を呼び出す前に、アクセストークンが環境変数HUGGINGFACE _トークンを使用してAPIが認証できるようにするトークン。
```
